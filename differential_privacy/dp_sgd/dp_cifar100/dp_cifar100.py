...
# ==============================================================================
"""Example differentially private trainer and evaluator for CIFAR100.

    Usage: (must set conv_layers=2 and disable pca projection)
        python dp_cifar100.py --save_path ../results_cifar \
...
DATA_DIR = '../../data/cifar100/cifar-100-binary'
NUM_TRAINING_IMAGES = 50000
NUM_TESTING_IMAGES = 10000
IMAGE_SIZE = 24

def Cifar100Input(eval=False):
    if not eval:
        images, labels = cifar100_input.distorted_inputs(data_dir=DATA_DIR,
                                                        batch_size=FLAGS.batch_size)
    else:
        images, labels = cifar100_input.inputs(eval_data=True,
                                              data_dir=DATA_DIR,
                                              batch_size=FLAGS.batch_size)
...
def Eval(network_parameters, num_testing_images, load_path, save_mistakes=False, use_eval_data=True):
    ...
    with tf.Graph().as_default(), tf.Session() as sess:
        ...
        images, labels = Cifar100Input(eval=use_eval_data)
...
def Train(network_parameters, num_steps, save_path, eval_steps=0):
...
def main(_):
    ...
    if FLAGS.num_conv_layers == 0:
        raise ValueError("Please set num_conv_layers=2 for cifar100")
